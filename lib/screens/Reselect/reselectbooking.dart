...
class _ReselectBookScreenState extends State<ReselectBookScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget._dateRange == null)
      widget._dateRange = DateTimeRange(
          start: DateTime.now(),
          end: DateTime(DateTime.now().year, DateTime.now().month,
              DateTime.now().day + 4));
    List<FlightInfo> flightListOutbound =
        generateFlightInfo(widget._destination, widget._dateRange.start, true);
    List<FlightInfo> flightListInbound =
        generateFlightInfo(widget._destination, widget._dateRange.start, false);
    List<HotelInfo> hotelList =
        generateHotelInfo(widget._destination, widget._dateRange);
...
SliverToBoxAdapter _buildTicket(
    double screenHeight,
    ...
    List<HotelInfo> hotelList) {
  bool isChecked = true;
  DateFormat dateFormatFullEn = DateFormat(
    'dd MMM yyyy',
  );
                      ...
                      child: TextButton(
                        onPressed: () {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => FlightSelection(
              ...
              children: <Widget>[
                ...
                Row(
                  children: <Widget>[
                    Text(
                      flightListInbound[
                              selectionProvider.getIndex('inboundFlight')]
                          .flightFrom,
                      style: TextStyle(
                          fontSize: 18,
                          fontWeight: FontWeight.bold,
            ...
            child: Column(
              children: <Widget>[
                ...
                Row(
                  ...
                  children: <Widget>[
                    Text(
                      hotelList[selectionProvider.getIndex('hotel')].hotelName,
                      style: TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.w600,
                          fontFamily: 'Pangram'),
                ...
                Row(
                  ...
                  children: [
                    Text(
                      hotelList[selectionProvider.getIndex('hotel')].roomType,
                      ...
                      child: TextButton(
                        onPressed: () {
                          Navigator.push(
                              context,
                              MaterialPageRoute(
                                  builder: (context) => HotelSelection(
                                        destination: destination,
                                        dateRange: dateRange,
                                      )));
                        },
